var chartUrl = "/mvc/yz/listZX17YZ?mode=0";
seriesCallback = function(result, series) {
	...
	series.push({name: '段位13', data: []});
	series.push({name: '段位14', data: []});
	series.push({name: '段位15', data: []});
	series.push({name: '段位16', data: []});
	return series;
}
seriesValueCallback = function(series, item) {
	var count = 0;
	var arr = ["w1", "w2", "w3", "w4", "w5", "w6", "w7", "w8", "w9", "w10", "w11", "w12", "w13", "w14", "w15", "w16"];
	for(i in arr) {		
