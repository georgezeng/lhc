...
public class TmFdYzDao {
	@Autowired
	private EntityManager em;

	public PageResult<TmFdYz> query(QueryInfo<TmFdYz> queryInfo) {
		...
		condition.append("from tm_fd_yz d").append("\n");
		List<Object> args = new ArrayList<Object>();
		if (queryInfo.getObject() != null) {
			condition.append(", (select yp.date from tm_fd_yz yp").append("\n");
			condition.append("where yp.year = ?").append("\n");
			condition.append("and yp.phase = ?) t").append("\n");
			TmFdYz yz = queryInfo.getObject();
			args.add(yz.getYear());
		...
		if (count > 0) {
			...
			Query query = em.createNativeQuery(sql.toString(), TmFdYz.class);
			...
			List<TmFdYz> list = query.getResultList();
			if (queryInfo.isToReverse()) {
				Collections.reverse(list);
			}
			return new PageResult<TmFdYz>(list, count, queryInfo.getPageInfo());
		}
		return new PageResult<TmFdYz>(new ArrayList<TmFdYz>(), 0, queryInfo.getPageInfo());
	}
}
