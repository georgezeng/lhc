...
public class BsYzDao {
	...
	public PageResult<BsYz> query(QueryInfo<BsYz> queryInfo) {
		...
		if (queryInfo.getObject() != null) {
			condition.append(", (select yp.date from bs_yz yp").append("\n");
			condition.append("where yp.year = ?").append("\n");
			condition.append("and yp.phase = ?) t").append("\n");
			BsYz yz = queryInfo.getObject();
			args.add(yz.getYear());
		...
		if (count > 0) {
			...
			Query query = em.createNativeQuery(sql.toString(), BsYz.class);
			...
			List<BsYz> list = query.getResultList();
			if (queryInfo.isToReverse()) {
				Collections.reverse(list);
			}
			return new PageResult<BsYz>(list, count, queryInfo.getPageInfo());
		}
		return new PageResult<BsYz>(new ArrayList<BsYz>(), 0, queryInfo.getPageInfo());
	}
}
