...
public class LhYzDao {
	@Autowired
	private EntityManager em;

	public PageResult<LhYz> query(QueryInfo<LhYz> queryInfo) {
		...
		condition.append("from lh_yz d").append("\n");
		List<Object> args = new ArrayList<Object>();
		if (queryInfo.getObject() != null) {
			condition.append(", (select yp.date from lh_yz yp").append("\n");
			condition.append("where yp.year = ?").append("\n");
			condition.append("and yp.phase = ?) t").append("\n");
			LhYz yz = queryInfo.getObject();
			args.add(yz.getYear());
		...
		if (count > 0) {
			...
			Query query = em.createNativeQuery(sql.toString(), LhYz.class);
			...
			List<LhYz> list = query.getResultList();
			if (queryInfo.isToReverse()) {
				Collections.reverse(list);
			}
			return new PageResult<LhYz>(list, count, queryInfo.getPageInfo());
		}
		return new PageResult<LhYz>(new ArrayList<LhYz>(), 0, queryInfo.getPageInfo());
	}
}
