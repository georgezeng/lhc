...
public class QwYzDao {
	@Autowired
	private EntityManager em;

	public PageResult<QwYz> query(QueryInfo<QwYz> queryInfo) {
		...
		condition.append("from qw_yz d").append("\n");
		List<Object> args = new ArrayList<Object>();
		if (queryInfo.getObject() != null) {
			condition.append(", (select yp.date from qw_yz yp").append("\n");
			condition.append("where yp.year = ?").append("\n");
			condition.append("and yp.phase = ?) t").append("\n");
			QwYz yz = queryInfo.getObject();
			args.add(yz.getYear());
		...
		if (count > 0) {
			...
			Query query = em.createNativeQuery(sql.toString(), QwYz.class);
			...
			List<QwYz> list = query.getResultList();
			if (queryInfo.isToReverse()) {
				Collections.reverse(list);
			}
			return new PageResult<QwYz>(list, count, queryInfo.getPageInfo());
		}
		return new PageResult<QwYz>(new ArrayList<QwYz>(), 0, queryInfo.getPageInfo());
	}
}
