...
public class QqYzDao {
	@Autowired
	private EntityManager em;

	public PageResult<QqYz> query(QueryInfo<QqYz> queryInfo) {
		...
		condition.append("from qq_yz d").append("\n");
		List<Object> args = new ArrayList<Object>();
		if (queryInfo.getObject() != null) {
			condition.append(", (select yp.date from qq_yz yp").append("\n");
			condition.append("where yp.year = ?").append("\n");
			condition.append("and yp.phase = ?) t").append("\n");
			QqYz yz = queryInfo.getObject();
			args.add(yz.getYear());
		...
		if (count > 0) {
			...
			Query query = em.createNativeQuery(sql.toString(), QqYz.class);
			...
			List<QqYz> list = query.getResultList();
			if (queryInfo.isToReverse()) {
				Collections.reverse(list);
			}
			return new PageResult<QqYz>(list, count, queryInfo.getPageInfo());
		}
		return new PageResult<QqYz>(new ArrayList<QqYz>(), 0, queryInfo.getPageInfo());
	}
}
