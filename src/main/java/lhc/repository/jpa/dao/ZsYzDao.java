...
public class ZsYzDao {
	@Autowired
	private EntityManager em;

	public PageResult<ZsYz> query(QueryInfo<ZsYz> queryInfo) {
		...
		condition.append("from zs_yz d").append("\n");
		List<Object> args = new ArrayList<Object>();
		if (queryInfo.getObject() != null) {
			condition.append(", (select yp.date from zs_yz yp").append("\n");
			condition.append("where yp.year = ?").append("\n");
			condition.append("and yp.phase = ?) t").append("\n");
			ZsYz yz = queryInfo.getObject();
			args.add(yz.getYear());
		...
		if (count > 0) {
			...
			Query query = em.createNativeQuery(sql.toString(), ZsYz.class);
			...
			List<ZsYz> list = query.getResultList();
			if (queryInfo.isToReverse()) {
				Collections.reverse(list);
			}
			return new PageResult<ZsYz>(list, count, queryInfo.getPageInfo());
		}
		return new PageResult<ZsYz>(new ArrayList<ZsYz>(), 0, queryInfo.getPageInfo());
	}
}
